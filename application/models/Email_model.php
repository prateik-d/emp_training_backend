...
class Email_model extends CI_Model {
	...
	public function send_smtp_mail($msg=NULL, $sub=NULL, $to=NULL, $from=NULL) {
        ...
        */
		...
		$mail->Port = get_settings('smtp_port'); // or 587 for tls or 465 for ssl
		$mail->IsHTML(true);
		$mail->Username = get_settings('smtp_user');
		$mail->Password = get_settings('smtp_pass');
		$mail->SetFrom($from,get_settings('system_name'));
		$mail->AddReplyTo($from, get_settings('system_name'));
		$mail->Subject = $sub;
    ...
    public function send_smtp_mail_with_attachment($msg=NULL, $sub=NULL, $to=NULL, $from=NULL, $attachment=NULL) {
		//Load email library
		$this->load->library('email');
		...
		*/
		...
		$mail->Subject = $sub;
		$mail->Body = $htmlContent;
		$filename_array=explode('/', $attachment);
		$filename=$filename_array[(count($filename_array)-1)];
		$mail->AddAttachment($attachment, $name = $filename,  $encoding = 'base64', $type = 'application/pdf');
		$mail->AddAddress($to);
    ...
    public function send_mail_on_quiz_complete($mail_subject = "", $mail_body = "", $attachment="")
    {
		$instructor_id		 = 0;
		...
		//$this->send_smtp_mail($mail_body, $mail_subject, $user_details['email'], $email_from);
		//$this->send_smtp_mail_with_attachment($mail_body, $mail_subject, 'pangrekarsandesh@gmail.com', $email_from, $attachment);
		$this->send_smtp_mail_with_attachment($mail_body, $mail_subject, $user_details['email'], $email_from, $attachment);
	}
