...
class Admin extends CI_Controller {
  ...
  public function get_courses() {
    ...
    if(!empty($courses)) {
      ...
      ';
        ...
        $nestedData['title'] = '<strong><a href="'.site_url('admin/course_form/course_edit/'.$row->id).'">'.ellipsis($row->title).'</a></strong><br>
  ...
  public function payment_success($payment_type = "", $payment_id = "") {
    ...
    if ($payment_type == 'stripe') {
      ...
      // ++++++ Start of correction to get instructor id +++++++ 
      
      $this->db->select('user_id');
      $this->db->from('course');
      $this->db->where('id', $payment_details['course_id']);
      $course_query = $this->db->get();
      $course_data=$course_query->row();
      $instructor_id = $course_data->user_id;
      
      // ++++++ End of correction to get instructor id +++++++ 
      
      //Below line is old code.
