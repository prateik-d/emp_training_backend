    ...
    <div class="" id="video" <?php if($lesson_details['lesson_type'] != 'video'):?> style="display: none;" <?php endif; ?>>
        ...
        <div class="form-group">
            ...
            <select class="form-control select2" data-toggle="select2" name="lesson_provider" id="lesson_provider" onchange="check_video_provider(this.value)">
                ...
                <option value="video_upload" <?php if(strtolower($lesson_details['video_type']) == 'video_upload') echo 'selected'; ?>>Video Upload</option>
            </select>
        </div>
        ...
        <?php
         ...
         if($lesson_details['video_type']=="video_upload")
         {
             $video_upload_div_display='';
         }

         else
         {
            $video_upload_div_display='display:none;';
         }
        ...
        <div class="" id = "video_upload" style="<?= $video_upload_div_display; ?>">
            <div class="form-group">
                <label><?php echo get_phrase('video_upload_video_url'); ?></label>
                <!-- <input type="text" id = "video_upload_video_url" name = "video_upload_video_url" class="form-control"> -->
                <input type="file" name="video_upload_file" id="video_upload_file" class="form-control">
            </div>

            <div class="form-group">
                <label><?php echo get_phrase('video_upload_duration'); ?></label>
                <input type="text" class="form-control" data-toggle='timepicker' data-minute-step="5" name="video_upload_duration" id = "video_upload_duration" data-show-meridian="false" value="<?php echo $lesson_details['duration']; ?>">
            </div>

            <div class="form-group">
                <label><?php echo get_phrase('video_upload_thumbnail'); ?> <small>(<?php echo get_phrase('the_image_size_should_be'); ?>: 979 x 551)</small> </label>
                <div class="input-group">
                    <div class="custom-file">
                        <input type="file" class="custom-file-input" id="video_upload_thumbnail" name="video_upload_thumbnail" onchange="changeTitleOfImageUploader(this)">
                        <label class="custom-file-label" for="video_upload_thumbnail"><?php echo get_phrase('video_upload_thumbnail'); ?></label>
                    </div>
                </div>
            </div>
        </div> 
